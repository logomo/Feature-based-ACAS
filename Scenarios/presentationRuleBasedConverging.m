...
% Simulation run
    ...
    while missionControl1.isActive || missionControl2.isActive
        ...
        view(-35,49)
        axis([-2,42,-2,42,-5,5])
        Cmnf.exportFigure('UtmCooperativeConverging',k);
    end
...
% plot sequence
    ...
    figure(2)
        ...
        [newX,newY]=Cmnf.preparefillData([missionTime(1),missionTime(nCount)],[8,8]);
    ...
    Cmnf.exportFigure('UtmCooperativeConvergingPerformance');
...
%% Trajectory tracking performance
    ...
    Cmnf.exportFigure('UtmCooperativeConvergingUAV1PathFollowing');
    
    figure(4)
    missionControl2.plotRealvsPlanTrajectoryStatistics()
    title('Path following performance  UAV2')
    Cmnf.exportFigure('UtmCooperativeConvergingUAV2PathFollowing');
 ...
 %% computaiton Load
    ...
    mean(aat')*100
    Cmnf.exportFigure('RuleBasedConvergingComputationTime');
