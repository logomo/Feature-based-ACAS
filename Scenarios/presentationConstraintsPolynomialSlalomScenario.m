...
while ~missionControl1.finalWaypointReachedFlag
    ...
    figure(1);
    daspect([1,1,1]);
    view(89.9200,56.0800);
    %axis([-5,45,-5,45,-10,10])
    k=k+1;
    title(['Decision frame ',mat2str(k),'.']);
    drawnow
    Cmnf.exportFigure('ConstraintsPolynomialSlalom',k);
end
...
%% Draw Clash distance performance
    ...
    %Labeling
    title('Slalom avoidance performance')
    xlabel('UTM time [s]')
    ylabel('Performance distance [m]')
    legend('UAS [m]','Safety margin [m]','Body [m]') 
    hold off
    
    Cmnf.exportFigure('ConstraintsPolynomialSlalomPerformance');
